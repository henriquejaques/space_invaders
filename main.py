...
RED_SPACESHIP = transforms.scale(RED_SPACESHIP_IMAGE, SPACESHIP_WIDTH, SPACESHIP_HEIGHT)
...
def yellow_handle_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_a] and yellow.x - VEL > 0:  # LEFT
        yellow.x -= VEL
    if (
        keys_pressed[pygame.K_d] and yellow.x + VEL + yellow.width - 12 < BORDER.x
    ):  # RIGHT
        yellow.x += VEL
    if keys_pressed[pygame.K_w] and yellow.y - VEL > 0:  # UP
        yellow.y -= VEL
    if (
        keys_pressed[pygame.K_s] and yellow.y + VEL + yellow.height < HEIGHT - 12
    ):  # DOWN
        yellow.y += VEL
...
def red_handle_movement(keys_pressed, red):
    if keys_pressed[pygame.K_LEFT] and red.x - VEL > BORDER.x + BORDER.width:  # LEFT
        red.x -= VEL
    if keys_pressed[pygame.K_RIGHT] and red.x + VEL + red.width - 12 < WIDTH:  # RIGHT
        red.x += VEL
